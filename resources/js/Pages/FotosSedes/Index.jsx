...
export default function Dashboard({ auth, galleries,sedes }) {
    ...
    const save = (e) => {
        e.preventDefault();
        if (operation === 1) {
            post(route("AdministradorFotosSedes.store"), {
                preserveScroll: true,
                onSuccess: () => {
                    ok("Foto Guardada.");
                },
            ...
            post(route("AdministradorFotosSedes.updatepost", data.id), {
                preserveScroll: true,
                onSuccess: () => {
                    ok("Foto Actualizada.");
                },
    ...
    const eliminar = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                destroy(route("AdministradorFotosSedes.destroy", id), {
                    onSuccess: () => {
                        ok("Foto Eliminada.");
                    },
                    onError: () => {
...
<AuthenticatedLayout
    ...
    header={
        <h2 className="text-xl font-semibold leading-tight text-gray-800">
            Fotos de la sede {sedes[0].nombre}
        </h2>
    }
>
    <Head title="Fotos Sede" />
                    ...
                    {galleries.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="7" className="px-4 py-2 text-center">
                                No hay Fotos de la sede {sedes[0].nombre}
                            </td>
                        </tr>
                    )}
                </tbody>
    ...
    <Modal show={modal} onClose={closeModal}>
        ...
        <form onSubmit={save} className="p-6">
            <input type="text" value={data.fk_sede_id} name="fk_sede_id" hidden readOnly />
