...
export default function Dashboard({ auth, galleries,productos }) {
    ...
    const handleModal = (
        ...
        setOperation(op);
        if (op === 1) {
            setTitle("Agregar Foto Producto");
            setData({
                largeUrl: null,
                width: "",
                height: "",
    ...
    const save = (e) => {
        e.preventDefault();
        if (operation === 1) {
            post(route("AdministradorFotosProductos.store"), {
                preserveScroll: true,
                onSuccess: () => {
                    ok("Foto Guardada.");
                },
            ...
            post(route("AdministradorFotosProductos.updatepost", data.id), {
                preserveScroll: true,
                onSuccess: () => {
                    ok("Foto Actualizada.");
                },
    ...
    const eliminar = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                destroy(route("AdministradorFotosProductos.destroy", id), {
                    onSuccess: () => {
                        ok("Foto Eliminada.");
                    },
                    onError: () => {
...
<AuthenticatedLayout
    ...
    header={
        <h2 className="text-xl font-semibold leading-tight text-gray-800">
            Fotos del producto {productos[0].nombre}
        </h2>
    }
>
    <Head title="Fotos Producto" />
                    ...
                    {galleries.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="7" className="px-4 py-2 text-center">
                                No hay Fotos del producto {productos[0].titulo}
                            </td>
                        </tr>
                    )}
                </tbody>
    ...
    <Modal show={modal} onClose={closeModal}>
        ...
        <form onSubmit={save} className="p-6">
            <input type="text" value={data.fk_producto_id} name="fk_producto_id" hidden readOnly />
